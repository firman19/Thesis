...
L.TrackPlayBackControl = L.Control.extend({
  ...
  _initContainer: function () {
    ...
    this._container.setAttribute('id','leaflet-control-playback')
    ...
    this._playBtn = this._createButton('play', 'btn-stop', this._sliderContainer, this._play)
    this._restartBtn = this._createButton('replay', 'btn-restart', this._sliderContainer, this._restart)

    this._slider = this._createSlider('time-slider', this._sliderContainer, this._scrollchange)

    this._slowSpeedBtn = this._createButton('slow', 'btn-slow', this._sliderContainer, this._slow)
    this._infoSpeedRatio = this._createInfo('', `X${this.trackPlayBack.getSpeed()}`, 'info-speed-ratio', this._sliderContainer)
    this._quickSpeedBtn = this._createButton('quick', 'btn-quick', this._sliderContainer, this._quick)

    this._closeBtn = this._createButton('close', 'btn-close', this._sliderContainer, this._close)
  ...
  _createInfo: function (title, info, className, container) {
    let infoContainer = L.DomUtil.create('div', 'info-container', container)
    let infoEle = L.DomUtil.create('a', className, infoContainer)
    infoEle.innerHTML = info
    return infoEle
  },
...
L.trackplaybackcontrol = function (trackplayback, options) {
  return new L.TrackPlayBackControl(trackplayback, options)
}
